...
- **The breaker's half-open state admits exactly one trial call.** Every caller after the first is rejected until that probe reports back — otherwise the full pre-breaker load hits a service that has not been shown healthy yet. A success closes the breaker; a failure re-opens it for another full timeout. A trial whose caller never reports back is released after `timeout`, so a dropped probe cannot wedge the breaker shut.
